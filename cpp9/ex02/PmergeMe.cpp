...
/*   Updated: 2024/06/14 03:58:57 by fursini          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void PmergeMe::fillVector(char **av)
{
	for (int i = 1; av[i]; i++)
	{
		if (std::all_of(av[i], av[i] + strlen(av[i]), ::isdigit))
...
void PmergeMe::sortList(std::list<unsigned int> &list)
{
	...
	if (size <= 1)
		return ;
	int mid = size / 2;
	std::list<unsigned int> left(list.begin(), std::next(list.begin(), mid));
	std::list<unsigned int> right(std::next(list.begin(), mid), list.end());
...
void PmergeMe::execute(char **av)
{
	...
	std::cout << std::fixed << std::setprecision(10) << "Time to process a range of " << this->vector.size() << " elements with std::vector: " << time.count() << " s" << std::endl;
	...
	time = std::chrono::duration_cast<std::chrono::duration<double>>(end - start);
	std::cout << std::fixed << std::setprecision(10) << "Time to process a range of " << this->list.size() << " elements with std::list: " << time.count() << " s" << std::endl;
