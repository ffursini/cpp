...
void PmergeMe::execute(char **av) {
	...
	std::cout << std::fixed << std::setprecision(10) << "Time to process a range of " << this->vector.size() << " elements with std::vector: " << time << " s" << std::endl;
	...
	time = static_cast<double>(end - start) / CLOCKS_PER_SEC;
	std::cout << std::fixed << std::setprecision(10) << "Time to process a range of " << this->list.size() << " elements with std::list: " << time << " s" << std::endl;
}
